...
class ReportMgrBase(object):
    ...
    def report_step(self, lr, step, test_stats=None, valid_stats=None):
        """
        Report stats of a step

        Args:
            test_stats(Statistics): test stats
            valid_stats(Statistics): validation stats
            lr(float): current learning rate
        """
        self._report_step(
            lr, step, test_stats=test_stats, valid_stats=valid_stats)
...
class ReportMgr(ReportMgrBase):
    ...
    def _report_step(self, lr, step, test_stats=None, valid_stats=None):
        """
        See base class method `ReportMgrBase.report_step`.
        """
        if test_stats is not None:
            self.log('AbsSummarizer Train perplexity: %g' % test_stats.ppl())
            self.log('AbsSummarizer Train accuracy: %g' % test_stats.accuracy())
        ...
        if valid_stats is not None:
            self.log('AbsSummarizer Validation perplexity: %g' % valid_stats.ppl())
            self.log('AbsSummarizer Validation accuracy: %g' % valid_stats.accuracy())
    ...
    def output(self, step, num_steps, learning_rate, start):
        ...
        logger.info(
            ...
            % (step, num_steps,
               ...
               self.xent(),
               learning_rate,
               self.n_correct / (t + 1e-5),
               self.n_words / (t + 1e-5),
               time.time() - start))
        sys.stdout.flush()
        print('abstractive_layer:\tloss:', self.loss, '\tn_words:', self.n_words)
