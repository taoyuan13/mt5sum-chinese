...
from models.model_builder_LAI import AbsSummarizer
...
def validate(args, device_id, pt, step):
    ...
    valid_loss = abs_loss(symbols, model.config.vocab_size, device, train=True, label_smoothing=args.label_smoothing)
...
def test_text_abs(args, device, pt, step, text):
    # device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if pt != '':
        test_from = pt
    else:
...
def train_abs_single(args, device_id):
    ...
    model = AbsSummarizer(args, device, checkpoint, load_from_extractive, load_from_abstractive)
    if args.sep_optim:
    ...
    train_loss = abs_loss(symbols, model.config.vocab_size, device, train=True, label_smoothing=args.label_smoothing)
